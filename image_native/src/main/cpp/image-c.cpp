...
#include "com_alpha_f_image_CImage.h"
...
void cropImg(CImg<unsigned char> &img,  int left, int top, int width, int height, float angle,
                     float resizeScale,  int exifDegrees, int exifTranslation) {
    const int
            x0 = left, y0 = top,
    ...
    */
    ...
    const int
            ...
            size_z = -100, size_c = -100, interpolation_type = 2;
...
JNIEXPORT jboolean JNICALL Java_com_alpha_f_image_CImage_cropCImg
    ...
    try {
        ...
        cropImg(img, left, top, width, height, angle, resizeScale, exifDegrees, exifTranslation);
